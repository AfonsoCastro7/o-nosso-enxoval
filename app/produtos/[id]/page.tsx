...
export default function ProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const remove = async () => {
    if (!window.confirm(`Eliminar “${product.name}”? Esta ação não pode ser anulada.`)) return;
    setDeleting(true);
    setActionError("");
    try {
  ...
  return <>
    ...
    <div className="card p-5">
      ...
      {product.description && <p className="mt-5 leading-7 text-slate-600">{product.description}</p>}
      <div className="mt-6 grid grid-cols-2 gap-4">
        <Info label={product.status === "bought" ? "Preço pago" : "Preço atual"} value={formatCurrency(product.price)} />
      ...
      {product.notes && <div className="mt-6 border-t border-slate-100 pt-5"><p className="label">Notas</p><p className="whitespace-pre-wrap text-sm leading-6 text-slate-600">{product.notes}</p></div>}
    ...
    <div className="mt-4 grid grid-cols-2 gap-3"><button className="button-secondary" onClick={() => setEditing(true)}><Pencil size={17} />Editar</button><button disabled={deleting} className="button-danger disabled:opacity-60" onClick={() => void remove()}><Trash2 size={17} />{deleting ? "A eliminar…" : "Eliminar"}</button></div>
  </>;
}

function Info({ label, value, icon }: { label: string; value: string; icon?: React.ReactNode }) {
  return <div><p className="label">{label}</p><p className="flex items-center gap-1.5 font-semibold text-slate-800">{icon}{value}</p></div>;
}
...
function BuyPanel({ onCancel, onSave }: { onCancel: () => void; onSave: (price: number, store: string, date: string) => Promise<void> }) {
  ...
  return <div className="card mt-4 space-y-4 border border-teal-100 p-5"><h2 className="font-bold">Concluir compra</h2><label><span className="label">Preço pago *</span><input autoFocus required type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label><label><span className="label">Loja</span><input value={store} onChange={(event) => setStore(event.target.value)} /></label><label><span className="label">Data da compra</span><input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label>{error && <p className="text-sm text-red-700">{error}</p>}<div className="flex gap-3"><button className="button-secondary flex-1" onClick={onCancel}>Cancelar</button><button disabled={price === "" || saving} className="button-primary flex-1 disabled:opacity-50" onClick={() => void save()}>{saving ? "A guardar…" : "Guardar"}</button></div></div>;
}
